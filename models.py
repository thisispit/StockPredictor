...
def prepare(df, symbol):
    ...
    # Drop rows where y is NaN, and also drop corresponding rows from X
...
def main():
    ...
    for fname in os.listdir('processed'):
        if fname.lower().endswith('_features.csv'):
            sym = fname.split('_')[0]
            df = pd.read_csv(os.path.join('processed', fname), parse_dates=['Date'])
            print(df.head())
            X,y,meta = prepare(df,sym)
            print(f"Symbol: {sym}, len(X): {len(X)}, len(y): {len(y)}")
            if len(X) < 5:
